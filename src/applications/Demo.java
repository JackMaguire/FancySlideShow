...
public class Demo {
	...
	public static void main( String[] args ) {
		...
		start_node.setImageFilename( filename_prefix_ + "/Track1/0001.png" );
		// File f = new File("src/PicsForDemo/Track1/0001.png");
		my_graph.setNode( start_node, 0 );

		my_graph.setNode( new ConceptualNode( "branch", true, filename_prefix_ + "Track1/0130.png" ), 1 );
		my_graph.setNode(
				new ConceptualNode( "end1", true, filename_prefix_ + "Track1/0250.png", "Don't forget to thank Yoda" ), 2 );
		my_graph.setNode( new ConceptualNode( "end2", true, filename_prefix_ + "Track2/0250.png" ), 3 );

		// my_graph.getNode( 0 )

		String[] filenames_for_start_to_branch = new String[ 128 ];
		for( int i = 0; i < 128; ++i ) {
			filenames_for_start_to_branch[ i ] = filename_prefix_ + "Track1/0" + String.format( "%03d", i + 2 ) + ".png";
		}
		my_graph.addEdge( new ConceptualEdge( "", 0, 1, filenames_for_start_to_branch ) );

		String[] filenames_for_branch_to_end1 = new String[ 119 ];
		for( int i = 0; i < 119; ++i ) {
			filenames_for_branch_to_end1[ i ] = filename_prefix_ + "Track1/0" + String.format( "%03d", i + 131 ) + ".png";
		}
		my_graph.addEdge( new ConceptualEdge( "", 1, 2, filenames_for_branch_to_end1 ) );

		String[] filenames_for_branch_to_end2 = new String[ 119 ];
		for( int i = 0; i < 119; ++i ) {
			filenames_for_branch_to_end2[ i ] = filename_prefix_ + "Track2/0" + String.format( "%03d", i + 131 ) + ".png";
		}
